...
export function ClientNutritionView({ clientId }: ClientNutritionViewProps) {
  ...
  async function fetchNutritionData(selectedDate: Date) {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        appToast.error(
          "You don't have permission to view this client's nutrition data",
        );
      } else if (status === 404) {
  ...
  const macroData = nutritionData
    ? [
        {
          name: "Protein",
          value: nutritionData.summary.totalProtein,
          color: COLORS.protein,
        },
        {
          name: "Carbs",
          value: nutritionData.summary.totalCarbs,
          color: COLORS.carbs,
        },
        {
          name: "Fat",
          value: nutritionData.summary.totalFat,
          color: COLORS.fat,
        },
      ]
    : [];
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {Math.round(
                    ((nutritionData.summary.totalProtein * 4) /
                      nutritionData.summary.totalCalories) *
                      100,
                  )}
                  % of calories
                </p>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {Math.round(
                    ((nutritionData.summary.totalCarbs * 4) /
                      nutritionData.summary.totalCalories) *
                      100,
                  )}
                  % of calories
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {Math.round(
                    ((nutritionData.summary.totalFat * 9) /
                      nutritionData.summary.totalCalories) *
                      100,
                  )}
                  % of calories
