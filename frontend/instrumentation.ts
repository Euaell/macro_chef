    ...
    if (!process.env.NEXT_BUILD) {
      ...
      if (tracingEndpoint || lokiEndpoint) {
        try {
          ...
          sdkConfig.metricReader = new PrometheusExporter({ port: metricsPort });
          ...
          console.log(`[OTel] Mizan.Frontend telemetry initialized (traces: ${!!tracingEndpoint}, logs: ${!!lokiEndpoint}, metrics: :${metricsPort})`);
        } catch (error) {
